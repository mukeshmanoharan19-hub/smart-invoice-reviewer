...
- A 13-document fictional multilingual corpus containing 12 invoices and one imperfect Hindi fuel receipt.
...
Document Intelligence stays primary. An LLM value can fill a missing field, but it cannot replace a conflicting primary value, validate GSTIN, or decide approval. Maya sees where every fallback came from.
