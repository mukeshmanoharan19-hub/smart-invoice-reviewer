...
The extraction evaluator routes each manifest entry to `prebuilt-invoice` or `prebuilt-receipt`, compares normalized fields, and continues past individual provider failures. Running it consumes 13 Azure analyze transactions across 14 pages. `scripts/evaluate_hybrid.py` runs the Hindi invoice and fuel receipt through both extraction methods and reports primary fields, LLM fallbacks, conflicts, final status, and call counts.

The committed set contains eleven PDFs and two PNG images. GSTIN values are fictional checksum examples and are never presented as verified business registrations.
